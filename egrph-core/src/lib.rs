...
mod tests {
    ...
    fn test_match_multiple_relationship_types_or() {
        ...
        let result3 = g
            .execute(
                "MATCH (a:Person)-[:KNOWS|LIKES|HATES]->(b:Person) RETURN a.name ORDER BY a.name",
            )
            .unwrap();
        ...
        assert_eq!(
            result4.rows[0].values[0],
            CypherValue::String("KNOWS".to_string())
        );
        assert_eq!(
            result4.rows[1].values[0],
            CypherValue::String("LIKES".to_string())
        );
    }
    ...
    // --- Pattern property predicate tests ---
    ...
    #[test]
    fn test_node_pattern_property_no_label() {
        let mut g = Graph::new();
    ...
    #[test]
    fn test_pattern_property_invalid_syntax_rejected() {
        // {key > value} is not valid Cypher property predicate syntax;
        // the grammar requires `key: value` format.
        let mut g = Graph::new();
        let err = g.execute("MATCH (n {age > 18}) RETURN n").unwrap_err();
        assert!(
